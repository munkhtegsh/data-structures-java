...
//    	printFolderContentsRecursive(topFolder);
    ...
    public static void printFolderContentsWithStack(Folder topFolder) {
        ...
        Stack<Folder> subFolderStack = new Stack<>();
        int indentFolder = 0;
        int indentSubFolder = 0;

        while (!folderStack.isEmpty()) {
            System.out.println(indentFolder + indentSubFolder + " " + folderStack.peek());
            Folder root = folderStack.pop();
			if (root.getFileList().size() != 0) {
				System.out.println(root.getFileList());
			}

            if (root.getFolderList().size() != 0) {
                for (Folder folder : root.getFolderList()) {
                    subFolderStack.push(folder);
                }

                while (!subFolderStack.isEmpty()) {
                    Folder subFolder = subFolderStack.pop();
                    folderStack.push(subFolder);
                }

                // when subfolder is empty add indent
                indentFolder++;
