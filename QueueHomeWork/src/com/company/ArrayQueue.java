...
import com.company.EmptyQueueException;
...
public final class ArrayQueue<T> implements QueueInterface<T> {
	...
	public void splice(ArrayQueue<T> anotherQueue) {
			int len = anotherQueue.queue.length;
			for (int i = anotherQueue.frontIndex; i != (anotherQueue.backIndex + 1) % len; i = (i + 1) % len  ) { // VERY IMPORTANT to understand
				ensureCapacity();
				backIndex = (backIndex + 1) % queue.length; // !!!!!!!!! WATCH OUT WITH THE EMPTY QUEUE => because another queue needs to fill out the nulls first
				queue[backIndex] = anotherQueue.queue[i];
			}
	}
