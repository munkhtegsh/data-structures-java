...
public final class ArrayQueue<T> implements QueueInterface<T> {
	...
	public void splice(ArrayQueue<T> secondQueue) {
			if (secondQueue.queue[0] != null) { // check the second queue has null items
				for (int i = secondQueue.frontIndex; i <= secondQueue.backIndex; i++) {
					ensureCapacity();
					backIndex++;
					queue[backIndex] = secondQueue.queue[i];
				}
